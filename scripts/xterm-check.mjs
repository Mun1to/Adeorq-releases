...
// -- EL COLOCADO QUE LLEGA TARDE: espera si hay un ciclo nuevo abierto -------
//
// En la app el colocado del cierre viaja en un setTimeout(0). Si un trozo del
// PTY trae [cierre del ciclo 1 + apertura y MEDIO ciclo 2] y el siguiente trozo
// tarda, ese macrotask aterriza con el bufer del ciclo 2 a medio crecer:
// colocar ahi contra un baseY parcial te dejaba varado historial arriba, y
// encima consumia la distancia y el cierre de verdad ya no colocaba nada. La
// regla: el colocado NO corre (ni consume) mientras un ?2026h siga abierto;
// lo relanza el ?2026l de ese ciclo. Este bloque replica esa maquina.
{
  const t = new Terminal({ cols: 100, rows: 24, scrollback: 8000, allowProposedApi: true });
  let pendiente = null;
  let abierto = false;
  ...
  const colocar = (aunqueAbierto = false) => {
    if (abierto && !aunqueAbierto) return;
  ...
  const macrotarea = () => new Promise((r) => setTimeout(r, 0));
  ...
  // Ciclo 1 entero, y pegado a el la apertura y la mitad del ciclo 2: el mismo
  // trozo, como lo entrega el PTY cuando Claude repinta varias veces por
  // segundo. Despues, silencio: el setTimeout(0) del cierre 1 aterriza aqui.
  const ciclo2 = lineas(620, "conversacion");
  const mitad = Math.floor(ciclo2.length / 2);
  t.write(
    "\x1b[?2026h\x1b[H\x1b[2J\x1b[3J" + lineas(610, "conversacion") + "\x1b[?2026l" +
    "\x1b[?2026h\x1b[H\x1b[2J\x1b[3J" + ciclo2.slice(0, mitad),
  );
  await vaciar(t);
  await macrotarea(); // el colocado del ciclo 1 dispara AHORA, en falso
  ok("con el ciclo 2 abierto, el colocado espera y no consume", pendiente !== null);

  t.write(ciclo2.slice(mitad) + "\x1b[?2026l");
  await vaciar(t);
  await macrotarea(); // y este es el bueno, con el bufer entero
  const b = t.buffer.active;
  ok(
    "el cierre de verdad te deja a seis del final del bufer ENTERO",
    b.baseY - b.viewportY === 6,
    `a ${b.baseY - b.viewportY}, esperaba 6`,
  );
  ok("y la distancia quedo consumida", pendiente === null);
